...
void UartInit(UART_TypeDef* UARTx,int BaudRate)
{
  ...
  GPIO_Init(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x0000)), &GPIO_InitStructure);
	...
	GPIO_PinAFConfig(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x0000)), ((uint16_t)0x0200) | ((uint16_t)0x0400), ((uint8_t)0x08)); 
	
	RCC_APB2PeriphClockCmd(((uint32_t)0x00000004), ENABLE);
	UART_InitStructure.UART_BaudRate = BaudRate; 
  UART_InitStructure.UART_WordLength = ((uint16_t)0x0030);
  UART_InitStructure.UART_StopBits = ((uint16_t)0x0000);
  UART_InitStructure.UART_Parity = ((uint16_t)0x0000) ;
  UART_InitStructure.UART_Mode = ((uint16_t)0x0008) | ((uint16_t)0x0010);
...
u8 Uart1_Receive(void)
{
  ...
  while((((UART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0800))->CSR & (1<<1)) ==0);
	return ((UART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0800))->RDR;

}

void send_data(u8 data)
{
	while((((UART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0800))->CSR &0x1) == 0);
	((UART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0800))->TDR = data; 
}
void send_group(u8*data,u16 len)
{
	while(len--)
...
{      
	while((((UART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0800))->CSR &0x1) == 0){}
		((UART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0800))->TDR = (u8) ch;      
	return ch;
}
