...
void SPI_DeInit(SPI_TypeDef* SPIx)
{
  ...
  {
  case ((((uint32_t)0x40000000) + 0x10000) + 0x5000):
...
void SPI_StructInit(SPI_InitTypeDef* SPI_InitStruct)
{
  ...
  SPI_InitStruct->SPI_DataWidth = ((uint16_t)0x0008);
   
  SPI_InitStruct->SPI_CPOL = ((uint16_t)0x0000);
   
  SPI_InitStruct->SPI_CPHA = ((uint16_t)0x0001);
   
  SPI_InitStruct->SPI_NSS = ((uint16_t)0x0000);
   
  SPI_InitStruct->SPI_BaudRatePrescaler = ((uint16_t)0x0002);
...
void SPI_Cmd(SPI_TypeDef* SPIx, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    SPIx->GCTL |= ((uint16_t)0x0001);
  }
  else
  {
     
    SPIx->GCTL &= ((uint16_t)0xFFFE);
  }
}
...
void SPI_ITConfig(SPI_TypeDef* SPIx, uint8_t SPI_IT, FunctionalState NewState)
{
  ...
  else
  {
    ...
    SPIx->GCTL &= (uint16_t)~((uint16_t)0x0002);
  }
...
void SPI_BiDirectionalLineConfig(SPI_TypeDef* SPIx, uint16_t SPI_Direction)
{
  ...
  if(SPI_Direction==((uint16_t)0x0008))
  {
    SPIx->GCTL |= ((uint16_t)0x0008);
  }
   
  if(SPI_Direction==((uint16_t)0x0010))
  {
    SPIx->GCTL |= ((uint16_t)0x0010);
  }
   
  if(SPI_Direction==((uint16_t)0xfff7))
  {
    SPIx->GCTL &= ((uint16_t)0xfff7);
  }
   
  if(SPI_Direction==((uint16_t)0xffef))
  {
    SPIx->GCTL &= ((uint16_t)0xffef);
  }
}
